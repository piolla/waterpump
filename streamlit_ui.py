...
...
    ...
    def run(self):
        ...
        if self.data is None:
            st.info("JSON 파일을 업로드하거나 샘플 데이터를 사용하세요.")
            self.load_data()
            return
        ...
        with col1:
            ...
            if st.button("📥 JSON 파일 다운로드"):
                ...
                st.download_button(
                    label="분석 결과 다운로드",
                    data=json_str,
                    file_name=f"water_pump_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
        ...
        with col2:
            ...
            if st.button("📋 리포트 생성"):
                ...
                st.download_button(
                    label="요약 리포트 다운로드",
                    data=report,
                    file_name=f"pump_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt",
                    mime="text/plain"
                )
